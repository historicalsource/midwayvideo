...
**
** $Header: /video/include/glide251/fxglide.h 1     4/20/98 1:21p Mlynch $
** $Log: /video/include/glide251/fxglide.h $
 * 
 * 1     4/20/98 1:21p Mlynch
 * Initial entry to source safe.
** 
...
#define kMaxVertexParam (20 + (12 * GLIDE_NUM_TMU) + 3)
  ...
  struct cmdTransportInfo {
    ...
    FxI32  roomToReadPtr;/* Last known hw ptr */
    FxI32  roomToEnd;    /* # of bytes until last usable address before fifoEnd */

    FxBool fifoLfbP;     /* Do we expect lfb writes to go through the lfb? */
    FxBool lfbLockCount; /* Have we done an lfb lock? Count of the locks. */
    
#if GLIDE_DISPATCH_SETUP
    FxI32 (FX_CSTYLE *triSetupProc)(const GrVertex* a, const GrVertex* b, const GrVertex* c);
  ...
  union hwDep_u {
    ...
    struct cvgDep_s {
      ...
      FxU32 curRenderBuf;  /* Cached value of the current buffer swapped by the
...
#define TRISETUP_NORGB _trisetup_cull
...
#define TRISETUP_RGB   _trisetup_rgb
#define TRISETUP_ARGB  _trisetup_argb
...
#define GR_CHECK_FIFO_PTR() \
  ...
  if ((gc != NULL) && (gc->cmdTransportInfo.fifoPtr != NULL)) \
...
/* If going through simulator make sure it sees the writes since our
 * 'hw pointer' is not a real address. Otherwise, if we have the
 * debugging fifo on then snoop the fifo writes.  
 */
#if HAL_CSIM
#undef GET
#undef GET16
...
#elif USE_PACKET_FIFO && GLIDE_USE_DEBUG_FIFO 
#undef SET
#define SET(d, s) \
do { \
  GR_DCL_GC; \
