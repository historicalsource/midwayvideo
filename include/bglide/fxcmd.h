...
** $Revision: 1 $ 
** $Date: 9/02/98 12:36a $ 
** $Log: /Releases/Banshee/GLOP/3Dfx/Devel/H3/glide3/src/fxcmd.h $
** 
...
extern void _reg_group_begin_internal_wax( FxU32 chipId, FxU32 regBase,
                                           FxU32 groupNum,             
                                           FxU32 groupMask,            
                                           FxU32 pktHdr,               
                                           FxU32 checkP,               
...
#  define GET16(s) s
#  define SET(d, s)    d = s
#  define SET16(d, s)  (* (FxU16 *) &d) = (FxU16) s
...
GR_ASSERT(((__groupMask) & (SSTCP_PKT4_MASK >> SSTCP_PKT4_MASK_SHIFT)) != 0x00); \
GR_SET_EXPECTED_SIZE(sizeof(FxU32) * (__groupNum), 1); \
REG_GROUP_BEGIN_INTERNAL_WAX(\
__chipId, __regBase, __groupNum, __groupMask, \
...
#define REG_GROUP_BEGIN_INTERNAL_WAX(__chipId, __regBase, __groupNum,      \
                                     __groupMask, __pktHdr, __checkP)      \
{                                                                          \
    GR_DCL_GC;                                                             \
    volatile FxU32* _regGroupFifoPtr = gc->cmdTransportInfo.fifoPtr;       \
    REG_GROUP_DCL_WAX(__groupMask, __regBase, __groupNum, __checkP);       \
    DBG_CALL( _reg_group_begin_internal_wax( __chipId, offsetof(SstGRegs, __regBase), __groupNum,\
                                             __groupMask, __pktHdr, __checkP,\
                                             _regGroupFifoPtr ) );          \
    SET(*_regGroupFifoPtr++, (__pktHdr));                                   
...
do {                                                                     \
  ...
  if ( !gc->windowed ) {                                                 \
    ...
    GR_ASSERT(gc->cRegs->cmdFifo0.readPtrL >=                            \
  ...
  GDBG_INFO(120, "\tregGroupFifoPtr: %p\n", _regGroupFifoPtr);\
        GDBG_INFO(120, "\taMin:               0x%x\n", gc->cRegs->cmdFifo0.aMin);\
        GDBG_INFO(120, "\taMax:               0x%x\n", gc->cRegs->cmdFifo0.aMax);\
        GDBG_INFO(120, "\tholeCount:          0x%x\n", gc->cRegs->cmdFifo0.holeCount);\
