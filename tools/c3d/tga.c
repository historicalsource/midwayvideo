...
#ifdef INCLUDE_SSID
char *ss_tga_c = "$Workfile: $ $Revision: $";
#endif
...
#endif
static void read_type1(char *tga_fname, FILE *fp, tga_header *header, tga_pixel **image_data);
static void read_type2(char *tga_fname, FILE *fp, tga_header *header, tga_pixel **image_data);
static void write_type2(FILE *fp, tga_header *header, tga_pixel *image_data);
static void read_type9(char *tga_fname, FILE *fp, tga_header *header, tga_pixel **image_data);
static void read_type10(char *tga_fname, FILE *fp, tga_header *header, tga_pixel **image_data);
...
void tga_read(char *tga_fname, tga_header *header, tga_pixel **image_data)
{
	...
	if (fp == NULL)
		...
		case 0:
			...
			break;
		case 1:
			read_type1(tga_fname, fp,header, image_data);
			break;
		case 2:
			read_type2(tga_fname, fp, header, image_data);
			break;
		case 3:
			util_error("%s is a type 3 black and white targa file, which is not supported\n", tga_fname);
			break;
		case 9:
			read_type9(tga_fname, fp, header, image_data);
			break;
		case 10:
			read_type10(tga_fname, fp, header, image_data);
			break;
		case 11:
