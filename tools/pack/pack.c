/*
 *		$Archive: /video/tools/pack/pack.c $
 *		$Revision: 1 $
 *		$Date: 10/02/97 6:10p $
 *
...
static void set_span_height(int *y_level_map, int start_index, int span_width, int value)
{
	...
	ASSERT(start_index + span_width <= page_right);
...
static void calc_stats(pack_stats *stats)
{
	...
	pix_count = stats->page_count * params->page_phy_width * stats->line_count;
 ...
 * The 2D block oriented packer library source
 */
