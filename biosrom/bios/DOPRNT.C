...
int	(*myputc)(char) = dputc;

#define	PUTC(ch)	(void)myputc(ch)
...
static cvtl(double number, int prec, int flags, char *signp, unsigned char fmtch, char *startp, char *endp);
static char *roundl(double fract, int *expv, char *start, char *end, char ch, char *signp);
static char *exponentl(char *p, int expv, unsigned char fmtch);
static int isspeciall(double d, char *bufp);

static char NULL_REP[] = "(null)";
static char buf[BUF];				/* space for %c, %[diouxX], %[eEfgG] */

int doprnt(const char *fmt0, va_list argp)
{
	...
	double _ldouble;			/* float and double precision arguments %L.[eEfgG] */
	...
	for (cnt = 0;; ++fmt)
	{
		...
		for (t = (char *)fp->_ptr; (ch = *fmt) && ch != '%'; ++cnt, ++fmt)
			if ((--n < 0 && (!(fp->_flag & _IOLBF) || -n >= fp->_bufsiz)) || (ch == '\n' && fp->_flag & _IOLBF))
			{
				fp->_cnt = n;
				fp->_ptr = t;
...
rflag:
		switch (*++fmt)
		{
			...
			case 'G':
//				if (flags & LONGDBL)
					_ldouble = va_arg(argp, double);
//				else
//					_ldouble = (double)va_arg(argp, float);
				/*
...
static double ptenneg[] =
{
  1e-1, 1e-2, 1e-4, 1e-8, 1e-16, 1e-32, 1e-64, 1e-128, 1e-256,
  1e-512, 1e-1024, 1e-2048, 1e-4096
};

#define MAXP 4096
#define NP   12
#define P    (4294967296.0 * 4294967296.0 * 2.0)   /* 2^65 */
static long double INVPREC = P;
static long double PREC = 1.0/P;
#undef P
/*
...
static int cvtl(double number, int prec, int flags, char *signp, unsigned char fmtch, char *startp, char *endp)
{
	char *p, *t;
	double fract;
...
eformat:
			if (expcnt)
			{
				*t++ = *++p;
				...
				{
					fract = 0;
					startp = roundl((double)0.0, &expcnt, startp, t - 1, *p, signp);
				}
...
static int isspeciall(double d, char *bufp)
{
	...
	__nan = 0;  /* don't assume the static is 0 (emacs) */
	if (ip->exp != 0x7ff)
		return(0);
	if ((ip->manh & 0x7ffff) || ip->manl)
	{
		strcpy(bufp, "NaN");
		__nan = 1;			/* kludge: we don't need the sign,  it's not nice but it should work */
	}
	else
